...
SELECT ?start ?goal ?pathExists ?attackRadius ?safePathExists ?roomsVisited
WHERE {
  # Ponto inicial
  ...
  OPTIONAL {
    ...
    # Verificar se há espaço suficiente para passar (diâmetro de Player1 = 1.0)
    BIND(?freeWidth > 1.0 || ?freeLength > 1.0 AS ?safePathExists)
  }
  
  # Determinar se o caminho existe e é seguro
  ...
  BIND(COALESCE(?safePathExists, false) AS ?safePathExists)
}